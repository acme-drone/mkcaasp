...
func (s *SaltCluster) SSHCmd(IP string, cmd ...string) *exec.Cmd {
	arg := append(
		[]string{"-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile /dev/null", "-i", "/home/atighineanu/work/CaaSP_kube/automation/caasp-openstack-terraform/ssh/id_caasp",
...
func (s *CAASPOut) SSHCommand(IP string, cmd ...string) *exec.Cmd {
	arg := append(
		[]string{"-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile /dev/null", "-i", "/home/atighineanu/work/CaaSP_kube/automation/caasp-openstack-terraform/ssh/id_caasp",
			fmt.Sprintf("root@%s", IP),
		},
...
func AdminOrchCmd(s *CAASPOut, option string, command string) (string, string) {
	...
	if option == "refresh" {
		...
		out, err := s.SSHCommand(s.IPAdminExt.Value, cmd...).CombinedOutput()
		if err != nil {
	...
	if option == "addrepo" {
		...
		out, err := s.SSHCommand(s.IPAdminExt.Value, cmdtorun...).CombinedOutput()
		if err != nil {
	...
	if option == "update" || option == "packupdate" {
		...
		if option == "update" {
			...
			cmd = s.SSHCommand(s.IPAdminExt.Value, cmdtorun...)
		} else {
